  ...
  if (pmbp->pmhd != nullptr) {
    ...
    if (pmbp->ppart != nullptr) {
      ...
      KOKKOS_LAMBDA(const int p) {
        ...
        pr(IPX,p) = (1. - rand)*size.d_view(m).x1min + rand*size.d_view(m).x1max;
        pr(IPX,p) = fmin(pr(IPX,p),size.d_view(m).x1max);
        pr(IPX,p) = fmax(pr(IPX,p),size.d_view(m).x1min);

        rand = rand_gen.frand();
        pr(IPY,p) = (1. - rand)*size.d_view(m).x2min + rand*size.d_view(m).x2max;
        pr(IPY,p) = fmin(pr(IPY,p),size.d_view(m).x2max);
        pr(IPY,p) = fmax(pr(IPY,p),size.d_view(m).x2min);

        rand = rand_gen.frand();
        pr(IPZ,p) = (1. - rand)*size.d_view(m).x3min + rand*size.d_view(m).x3max;
        pr(IPZ,p) = fmin(pr(IPZ,p),size.d_view(m).x3max);
        pr(IPZ,p) = fmax(pr(IPZ,p),size.d_view(m).x3min);
